...
static inline void cell_lock (struct cell *c)
{
...
static inline void cell_unlock (struct cell *c)
{
...
static void mq_init (struct message_queue *mq)
{
...
static int mq_pop (struct message_queue *mq, struct message *m)
{
	if (mq->head == mq->tail)
		return 1;
	*m = mq->queue[mq->head];
...
static struct cell *cell_create ()
{
...
static void cell_destroy (struct cell *c)
{
	assert (c->ref == 0);
	free (c->mq.queue);
	assert (c->L == NULL);
...
static void trash_msg (lua_State * L, struct cell *c)
{
